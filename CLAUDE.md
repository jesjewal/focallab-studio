...
- **Instrument layout (2026-07-11, merged from Jesse's Google Stitch explorations — brief at `docs/stitch-redesign-prompt.md`):** `#shell` = fixed left `#sideNav` (logo mark 58px + wordmark + icon section-nav buttons that scroll/expand/flash the target card, `.cur` = ink pill) + `#mainCol` (slim `#appBar`: letterspaced "FOCAL LAB / STUDIO" text + Learn left, toolbar right → `#content` → `#statusBar` footer with ● SYSTEM READY / © / build). `#content` = `#heroRow` grid (hero `#stage`: frame + 3-col stats strip BELOW it · `#readouts`: 4 big amber readout cards `#roF/#roN/#roS/#roDof`, updated at the end of `drawStats`) then `#modules` (**CSS multi-columns masonry** — `columns:3 300px`, cards `break-inside:avoid` — this is the "fold under to fill space" behavior Jesse asked for) then `#wideMods` (top view + about, full width). Breakpoints: ≤1150 readouts become a 4-across row; ≤900 sidebar hides (mark appears in barTitle via mask); ≤700 readouts 2×2. `#printSheet`/`#toast`/modals stay DIRECT children of `<body>` (print CSS relies on it). The full lockup image appears ONLY in og-card/README — not in the UI.
